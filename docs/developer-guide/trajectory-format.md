---
layout: docs
title: "فرمت مسیر حرکت"
permalink: /docs/developer-guide/trajectory-format/
---
...
Thetool_statsandtool_error_countsdictionaries are normalized to include
ALL possible tools (frommodel_tools.TOOL_TO_TOOLSET_MAP) with zero defaults,
ensuring consistent schema across entries for HuggingFace dataset loading.

`tool_stats`
`tool_error_counts`
`model_tools.TOOL_TO_TOOLSET_MAP`
...
converted to<think>viaconvert_scratchpad_to_think().
3. Empty think blocks: Everygptturn is guaranteed to have a<think>block. If no reasoning was produced, an empty block is inserted:<think>\n</think>\n— this ensures consistent format for training data.
...
Empty think blocks: Everygptturn is guaranteed to have a<think>block. If no reasoning was produced, an empty block is inserted:<think>\n</think>\n— this ensures consistent format for training data.
...
- The tool name is matched by position against the parent assistant'stool_callsarray
...
- Schema reference forFunctionCallobjects
- <tool_call>example
...
The normalizedtool_statsschema ensures all entries have the same columns,
...
Or via the--save-trajectoriesflag. When the agent initializes withsave_trajectories=True, the_save_trajectory()method is called at the end
